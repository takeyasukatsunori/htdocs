...
session_start();
if(empty($_SESSION['admin.login'] || $_SESSION['admin.login']!== true))
try {
...
if(!empty($_GET['message_id'])){
    ...
    //投稿を取得するコードが入る
    // SQL作成
	$stmt = $pdo->prepare("SELECT * FROM bbs WHERE id = :id");
...
<form method="post">
	<div>
		...
		<input id="view_name" type="text" name="view_name" value="<?php if( !empty($message_data['view_name']) ){ echo $message_data['view_name']; } ?>"></div>
	<div>
    ...
    <input type="hidden" name="message_id" id="<?php if( !empty($message_data['id']) ){ echo $message_data['id']; } ?>">
</form>
</body>
</html>